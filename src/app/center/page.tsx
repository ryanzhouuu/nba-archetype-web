...
export const metadata = { title: "Center Archetypes" };
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-b from-slate-950 to-slate-900">
        ...
        <div className="absolute inset-0 opacity-25 bg-[linear-gradient(120deg,rgba(3,105,161,0.10)_0%,transparent_30%,transparent_70%,rgba(34,211,238,0.10)_100%)]" />
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 pt-10 pb-6">
        <div className="flex items-center justify-between">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs text-gray-200">
            <span className="h-2 w-2 rounded-full bg-gradient-to-r from-sky-600 to-cyan-500" />
            Center
          </div>
        </div>
        <h1 className="mt-6 text-4xl sm:text-5xl font-extrabold text-white tracking-tight">
          Center Archetype Clusters
        </h1>
        <p className="mt-2 text-gray-300 max-w-2xl">
          Rim protection, interior finishing, and hub playmaking variants.
          Explore clustered play styles among Cs.
        </p>
      </header>
      <main className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 pb-16">
        <PositionClusters pos="C" />
      </main>
    </div>
  );
}
